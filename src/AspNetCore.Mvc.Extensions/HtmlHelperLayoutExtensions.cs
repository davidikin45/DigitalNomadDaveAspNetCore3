...
namespace AspNetCore.Mvc.Extensions
{
    public static class HtmlHelperLayoutExtensions
    {
        ...
        {
            var html = GoogleApi.Font.GetFontScriptAsync(fonts, regular, bold, black, italic, boldItalic, hideTextWhileLoading, timeout);
            return new HtmlString(html);
        }

        public static HtmlString GetFontBodyCSSAsync(this IHtmlHelper helper, string font)
        {
            return new HtmlString(GoogleApi.Font.GetFontBodyCSSAsync(font));
        }

        public static HtmlString GetFontNavBarCSSAsync(this IHtmlHelper helper, string font, string styleCSS)
        {
            return new HtmlString(GoogleApi.Font.GetFontNavBarCSSAsync(font, styleCSS));
        }
        ...
        {
            var html = GoogleApi.Font.GetFontLink(font, regular, bold, italic, boldItalic);
            if (bodyFont)
            {
                html += Environment.NewLine + GoogleApi.Font.GetFontBodyCSS(font);
            }

            if (navBarFont)
            {
                html += Environment.NewLine + GoogleApi.Font.GetFontNavBarCSS(font, styleCSS);
            }
        ...
        {
            var html = GoogleApi.Font.GetEffectsLink(effects);
            return new HtmlString(html);
        }
    }
}
