...
namespace AspNetCore.Mvc.Extensions.Data
{
    public abstract class DbSeedIdentity<TUser> where TUser : IdentityUser
    {
        ...
        public class SeedRole
        {
            ...
            public SeedRole(string name, params string[] permissions)
            {
                Name = name;
        ...
        public virtual void Seed(DbContextIdentityBase<TUser> context)
        {
        ...
        protected virtual void CreateRolePermissions(DbContextIdentityBase<TUser> context, IEnumerable<SeedRole> roles)
        {
            foreach (var role in roles)
            {
                foreach (var scope in role.Permissions)
                {
                    if (context.RoleClaims.Where(rc => rc.RoleId == role.Name && rc.ClaimType == "permission" && rc.ClaimValue == scope).FirstOrDefault() == null)
                    {
                        var claim = new IdentityRoleClaim<string>() { ClaimType = "permission", ClaimValue = scope, RoleId = role.Name };
                        context.RoleClaims.Add(claim);
                    }
                }
                var remove = context.RoleClaims.Where(rc => rc.RoleId == role.Name && rc.ClaimType == "permission" && !role.Permissions.Contains(rc.ClaimValue)).ToList();
