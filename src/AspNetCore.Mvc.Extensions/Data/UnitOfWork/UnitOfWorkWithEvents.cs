...
namespace AspNetCore.Mvc.Extensions.Data.UnitOfWork
{
    ...
    public abstract class UnitOfWorkWithEvents : UnitOfWork
    {
        ...
        private IEnumerable<ValidationResult> GetValidationErrors(DbContext context, DbContextDomainEventsEFCoreAdapter domainEvents, bool newChanges)
        {
        ...
        public override async Task<Result<int>> CompleteAsync(CancellationToken cancellationToken)
        {
            try
            {
                if (validateOnSave)
                {
                    ...
                    {
                        return Result.ValidationFailed<int>(validationResult.Errors);
                    }
                }
