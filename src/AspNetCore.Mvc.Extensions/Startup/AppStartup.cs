...
namespace AspNetCore.Mvc.Extensions
{
    ...
    public abstract class AppStartup
    {
        ...
        public AppStartup(IConfiguration configuration, Microsoft.AspNetCore.Hosting.IWebHostEnvironment hostingEnvironment, Action<AppStartupOptions> config = null)
        {
            ...
            Logger = new SerilogLoggerFactory().CreateLogger(this.GetType());
        ...
        public virtual void ConfigureSettingsServices(IServiceCollection services)
        {
            ...
            services.ConfigureSettings<OpenIdConnectSettings>(Configuration.GetSection("OpenIdConnectSettings"));
        ...
        public virtual void ConfigureAuthenticationServices(IServiceCollection services)
        {
            ...
            if (authenticationSettings.JwtToken.Enable)
            {
                Logger.LogInformation($"Configuring JWT Authentication" + Environment.NewLine +
                                       ...
                                       $"Audiences: {tokenSettings.Audiences ?? ""}" + Environment.NewLine +
                                       $"NameClaimType: {tokenSettings.NameClaimType ?? ""}" + Environment.NewLine +
                                       $"RoleClaimType: {tokenSettings.RoleClaimType ?? ""}"
                                       );
                ...
                authenticationBuilder.AddJwtAuthentication(
                   ...
                   tokenSettings.NameClaimType,
                   tokenSettings.RoleClaimType);
            }
            ...
            if (authenticationSettings.OpenIdConnect.Enable)
            {
                ...
                var openIdConnectSettings = GetSettings<OpenIdConnectSettings>("OpenIdConnectSettings");
                authenticationBuilder.AddOpenIdConnect(OpenIdConnectDefaults.AuthenticationScheme, options =>
                {
                    ...
                    options.UsePkce = true; //.NET Core 3.0 https://www.scottbrady91.com/OpenID-Connect/ASPNET-Core-using-Proof-Key-for-Code-Exchange-PKCE
                    options.ResponseType = OpenIdConnectResponseType.Code; //Authorization** Mitigates injection/substitution attack. Client-side just needs to generate random string and hash it using SHA256.

                    //options.ResponseType = OpenIdConnectResponseType.IdToken; //Implicit - Dont Use
                    //options.ResponseType = OpenIdConnectResponseType.IdTokenToken; //Implicit - Dont Use
                    //options.ResponseType = OpenIdConnectResponseType.CodeIdToken; //Hybrid MVC/Blazor Server** Mitigates injection/substitution attack but client-side code is more difficult to implement. Potentially leak identifiable information via the front-channel identity token.
                    //options.ResponseType = OpenIdConnectResponseType.CodeToken; ; //Hybrid - Dont Use
                    //options.ResponseType = OpenIdConnectResponseType.CodeIdTokenToken; ; //Hybrid - Dont Use
                    ...
                    //Identity Scopes
                    //options.Scope.Add("openid"); //added automatically
                    //options.Scope.Add("profile"); //added automatically
                    ...
                    foreach (var scope in openIdConnectSettings.IdentityScopes)
                    {
                        options.Scope.Add(scope);
                    }

                    //Refresh Token scope
                    if (openIdConnectSettings.RefreshTokens)
                    ...
                    //API Scopes
                    foreach (var scope in openIdConnectSettings.ApiScopes)
                    {
                        options.Scope.Add(scope);
                    }
                    ...
                    foreach (var claim in openIdConnectSettings.ClaimMappings)
                    {
                    ...
                    {
                        NameClaimType = openIdConnectSettings.NameClaimType,
                        RoleClaimType = openIdConnectSettings.RoleClaimType,
                    };
                ...
                {
                    client.BaseAddress = new Uri(openIdConnectSettings.Authority);
        ...
        public virtual void ConfigureAuthorizationServices(IServiceCollection services)
        {
            ...
            services.AddAuthorization(options =>
            {
                ...
                options.AddPolicy("UserMustBeAdmin", policyBuilder =>
                {
                    ...
                    //policyBuilder.RequireScope("scope2", "scope3"); //Dont use IdentityModel.AspNetCore.AccessTokenValidation
        ...
        public virtual void ConfigureMvcServices(IServiceCollection services)
        {
            ...
            //Dynamic Json not supported until 5.0. Maybe 6.0
            ...
            //ExpandoObject is a FULL implementation of IDynamicMetaObjectProvider, where the .NET Framework team has made all of these decisions for you.This is useful if you don't need any custom behavior, and you feel that ExpandoObject works good enough for you (90% of the time, ExpandoObject is good enough).
            //https://github.com/dotnet/runtime/issues/29690
            //https://github.com/dotnet/runtime/pull/42097
            ...
            services.Configure<JsonOptions>(options =>
            {
                ...
                options.JsonSerializerOptions.Converters.Add(new DynamicObjectConverter()); //JsonSerializer.Deserialize<dynamic> in 3.1 returns a JsonElement which DOES NOT implement IDynamicMetaObjectProvider. This returns an IDynamicMetaObjectProvider which does.
            });
            ...
            var mvc = services.AddMvc(options =>
            {
                ...
                options.Conventions.Add(new MvcAsApiConvention(o =>
                {
                    ...
                    {
                        mvcExceptionOptions.ActionResultFactories.Add(typeof(UnauthorizedException), (context, exception, logger) =>
                        {
                    ...
                    {
                        apiExceptionOptions.ActionResultFactories.Add(typeof(UnauthorizedException), (context, exception, logger) =>
                        {
                        ...
                        apiExceptionOptions.ActionResultFactories.Add(typeof(BadRequestException), (context, exception, logger) =>
                        {
                            var problemDetails = StaticProblemDetailsFactory.CreateProblemDetails(context.HttpContext, StatusCodes.Status400BadRequest, null, null, exception.Message);
                            return problemDetails.ToActionResult();
                        });
                        apiExceptionOptions.ActionResultFactories.Add(typeof(ValidationException), (context, exception, logger) =>
                        {
                            var problemDetails = StaticProblemDetailsFactory.CreateValidationProblemDetails(context.HttpContext, ((ValidationException)exception).Errors);
                            return problemDetails.ToActionResult();
                        });
                        apiExceptionOptions.ActionResultFactories.Add(typeof(NotFoundException), (context, exception, logger) =>
                        {
                            var problemDetails = StaticProblemDetailsFactory.CreateProblemDetails(context.HttpContext, StatusCodes.Status400BadRequest, null, null, exception.Message);
                            return problemDetails.ToActionResult();
                        });
        ...
        public virtual void ConfigureContextServices(IServiceCollection services)
        {
            ...
            services.AddTransient<ApplicationServiceServicesContext>();
        }
        #endregion
        ...
        public virtual void ConfigureApiServices(IServiceCollection services)
        {
            ...
            //Overrides the default ProblemDetailsFactory implementation which adds traceId, timeGenerated and exception details to the ProblemDetails response.
            builder.AddMvcEnhancedProblemDetailsFactory(options => options.EnableAngularErrors = true);
            //Overrides the default IClientErrorFactory implementation.
            builder.AddMvcEnhancedProblemDetailsClientErrorFactory();
            //400 vs 422
            builder.ConfigureMvcProblemDetailsInvalidModelStateFactory();

            builder.AddApiVersioning();
            builder.AddSwaggerWithApiVersioning(options =>
            {
                options.ApiTitle = $"{this.GetType().Assembly.GetName().Name} API";
                options.XmlCommentsFullPath = System.IO.Path.Combine(AppContext.BaseDirectory, $"{this.GetType().Assembly.GetName().Name}.xml");
            });
        ...
        public virtual void ConfigureHealthCheckServices(IServiceCollection services)
        {
            ...
            //.AddUrlGroup(new Uri(""), "API", HealthStatus.Degraded, timeout: TimeSpan.FromSeconds(1)) //AspNetCore.HealthChecks.Uris
            ...
            ISignalRHubMapper signalRHubMapper, ILoggerFactory loggerFactory, IApiVersionDescriptionProvider apiVersionDescriptionProvider)
        {
            ...
            if (!env.IsProduction())
            {
                ...
                {
                    appBranch.UseWhen(context => context.Request.IsMvc(), mvcBranch =>
                    {
                        mvcBranch.UseDeveloperExceptionPage(); // services.AddDatabaseDeveloperPageExceptionFilter();
                    ...
                    {
                        apiBranch.UseProblemDetailsExceptionHandler(options =>
                        {
                ...
                app.UseMigrationsEndPoint(); // services.AddDatabaseDeveloperPageExceptionFilter();
            ...
            {
                endpoints.MapHealthChecks("/hc/live", new HealthCheckOptions() { Predicate = _ => false, ResponseWriter = HealthCheckJsonResponseWriter.WriteResponse }).RequireAuthorization("HealthCheckPolicy");
                endpoints.MapHealthChecks("/hc/ready", healthCheckOptions).RequireAuthorization("HealthCheckPolicy");
                //.RequireCors(builder =>
                //{
