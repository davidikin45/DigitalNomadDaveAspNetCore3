...
namespace AspNetCore.Mvc.Extensions.Controllers.Api.Authentication
{
        ...
        where TUser : IdentityUser
    {
        ...
        public virtual async Task<IActionResult> Authenticate([FromBody] AuthenticateDto authenticateDto)
        {
        ...
        {
            //mimicking resource owner password flow

            var rolesAndPermissions = await AuthenticationHelper.GetRolesAndPermissionsAsync(user, _userManager, _roleManager);
            var roles = rolesAndPermissions.Roles;
            var permissions = rolesAndPermissions.Permissions;

            if (!string.IsNullOrWhiteSpace(_privateSigningKeyPath))
            {
                var key = SigningKey.LoadPrivateRsaSigningKey(_privateSigningKeyPath);
                var results = JwtTokenHelper.CreateJwtTokenSigningWithRsaSecurityKey(user.Id, user.UserName, user.Email, roles, _tokenExpiryMinutes, key, _localIssuer, _audience, permissions.ToArray());
            ...
            else if (!string.IsNullOrWhiteSpace(_privateSigningCertificatePassword))
            {
                ...
                var results = JwtTokenHelper.CreateJwtTokenSigningWithCertificateSecurityKey(user.Id, user.UserName, user.Email, roles, _tokenExpiryMinutes, key, _localIssuer, _audience, permissions.ToArray());
                return Created("", results);
            }
            else
            {
                var key = SigningKey.LoadSymmetricSecurityKey(_privateSymmetricKey);
                var results = JwtTokenHelper.CreateJwtTokenSigningWithKey(user.Id, user.UserName, user.Email, roles, _tokenExpiryMinutes, key, _localIssuer, _audience, permissions.ToArray());
                return Created("", results);
            }
        }
        #endregion
        ...
        public virtual async Task<IActionResult> ForgotPassword([FromBody] ForgotPasswordDto forgotPasswordDto)
        {
        ...
        public virtual async Task<IActionResult> ResetPassword([FromBody] ResetPasswordDto resetPasswordDto)
        {
